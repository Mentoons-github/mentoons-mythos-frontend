...
const MythosBanner = () => {
  ...
  return (
    <section className="w-full h-screen flex justify-center items-center py-6 sm:py-8 md:py-10 bg-[url('/assets/banner/Section.png')] bg-cover bg-center bg-no-repeat mulish px-3 sm:px-4 md:px-5 overflow-hidden">
      <div className="relative space-y-3 sm:space-y-4 md:space-y-5 p-3 sm:p-4 md:p-5 text-center flex flex-col pt-40 md:pt-0 justify-start items-start w-full">
        <motion.h1
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="text-[#E39712] font-semibold break-words md:w-[550px] text-left text-2xl sm:text-3xl md:text-4xl lg:text-5xl tracking-[1.5px] sm:tracking-[2.9px] font-montserrat"
        >
        ...
        >
          <MythosButton label="FIND YOUR PATH" bg="#FEE898" onClick={handlePathClick}/>
        </motion.div>
      </div>
    </section>
  );
