...
const ViewJob = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 bg-opacity-60 flex justify-center items-center z-50 ">
      <div className=" l shadow-lg p-4 md:p-8  bg-secondary w-full max-w-[350px] md:max-w-2xl lg:max-w-4xl max-h-[90vh] rounded-sm relative hide-scrollbar overflow-y-auto will-change-scroll transform-gpu">
        {/* Close button */}
        <button className="absolute top-4 right-4 hover:text-muted-foreground" onClick={onClose}>
          <X size={20} />
        </button>
          ...
          <div>
            <div className="text-center md:text-left">
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-3 md:gap-4 md:justify-start">
                <span className="px-3 py-1 text-xs  rounded-full bg-muted-foreground md:text-sm">
                  {job?.jobType}
                </span>
                <span className="px-3 py-1 text-xs rounded-full bg-muted-foreground md:text-sm">
                  {job?.jobLocation}
            ...
            <div className="mt-6 ">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {job?.skillsRequired.map((skill: string, index: number) => (
                  <span
                    key={index}
                    className="bg-muted-foreground px-3 py-1.5 rounded-lg text-xs md:text-sm"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="mt-5 text-lg md:text-xl flex gap-2">
